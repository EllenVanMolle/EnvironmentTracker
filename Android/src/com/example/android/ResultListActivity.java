...
public class ResultListActivity extends FragmentActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (findViewById(R.id.result_detail_container) != null) {
			...
			((ResultListFragment) getSupportFragmentManager().findFragmentById(
					R.id.result_list)).setActivateOnItemClick(true);
		}
	}
	...
	public void onItemSelected(String id) {
		if (mTwoPane) {
			...
			arguments.putString(ResultDetailFragment.ARG_ITEM_ID, id);
			ResultDetailFragment fragment = new ResultDetailFragment();
			fragment.setArguments(arguments);
			getSupportFragmentManager().beginTransaction()
					.replace(R.id.result_detail_container, fragment).commit();
